...
function lightsgoonoroff() {
	if(blackon) {
		...
		var borderscine = spotlightradius - 8;
		var mousespotlightstyle = '-moz-radial-gradient(-' + spotlightradius + 'px -' + spotlightradius + 'px, circle cover, rgba(0,0,0,0) ' + borderscine + 'px, ' + lightcolor + ' ' + spotlightradius + 'px)';
		...
		var style = '-moz-radial-gradient('+x+'px '+y+'px, circle cover, rgba(0,0,0,0) ' + borderscine + 'px, ' + lightcolor + ' ' + spotlightradius + 'px)';
		spot.style.opacity = default_opacity/100;
		spot.style.backgroundImage = style;
		}
		window.addEventListener("mousemove",function(event){try {moveSpot(event);}catch(err){}},false);
		}
...
/////////// Turn Off the Lights reader slider
	...
	if(window.devicePixelRatio >= 2){totlreaderbarimagelogo.src = "chrome://TurnOfftheLights/content/icons/icon16.png@2x.png";}
