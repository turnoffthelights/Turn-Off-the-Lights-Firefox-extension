...
// firefox check that it not run in "chrome" page
// check if not on firefox "chrome" page
var lightschecktaburl = window.content.location.protocol;
if(lightschecktaburl != "chrome:"){
...
// id way
if (!window.location.href.match(/http:\/\/(.*\.rtlxl\.nl\/.*|.*\.rtlxl\.com\/.*\/.*)/i)){
// skip the api on this page
// example http://www.rtlxl.nl/#!/rtl-z-nieuws-210334/eb1fffd2-cb63-3e05-88df-9f294e809486
// problem because call the  document.getElementById(
...
///////////
	...
	function reader() {
		...
		if (readerontext != null && readeronrange != null) {
			firefox.extension.sendRequest({name: "readersaveme", value : readerontext.value});
		}
		...
		if (readerlargestyle != null && readerlargeimgclick != null && readerlargetitleclick != null) {
			if(readerlargestyle.style.width == '24px'){firefox.extension.sendRequest({name: "readerlargestyle", value : false});}
			else{firefox.extension.sendRequest({name: "readerlargestyle", value : true});}
		}	
		}
...
mousespotlighto = response.mousespotlighto;
mousespotlightc = response.mousespotlightc;
mousespotlighta = response.mousespotlighta;
lightcolor = response.lightcolor;if(lightcolor)lightcolor = response.lightcolor;
lightimagea = response.lightimagea;
lightimage = response.lightimage;
interval = response.interval; default_opacity = interval;
fadein = response.fadein;
fadeout = response.fadeout;
readera = response.readera;
readerlargestyle = response.readerlargestyle;
mousespotlightt = response.mousespotlightt;
// ... see below ...
password = response.password; // old
...
try {
    ...
    if (!loginManager.getLoginSavingEnabled(username)) {
		return false;
    }
...
blur = response.blur;
cinemaontop = response.cinemaontop;
spotlightradius = response.spotlightradius;
slideeffect = response.slideeffect;
lightimagelin = response.lightimagelin;
...
var i18nlockentername = "Enter your password";
var i18nlockwrongpassword = "Wrong password";
...
function lightsgoonoroff() {
	if(blackon) {
		...
		var mousespotlightstyle = '-moz-radial-gradient(-' + spotlightradius + 'px -' + spotlightradius + 'px, circle cover, rgba(0,0,0,0) ' + borderspotlightsize + 'px, ' + lightcolor + ' ' + spotlightradius + 'px)';
		...
		var style = '-moz-radial-gradient(' + x + 'px ' + y + 'px, circle cover, rgba(0,0,0,0) ' + borderspotlightsize + 'px, ' + lightcolor + ' ' + spotlightradius + 'px)';
		...
		function spotmousedown() {
			...
			countupsizetimer = window.setInterval(function(){
				...
				style = '-moz-radial-gradient(' + x + 'px ' + y + 'px, circle cover, rgba(0,0,0,0) ' + mathbordersizeup + 'px, ' + lightcolor + ' ' + mathfullsizeup + 'px)';
				spot.style.backgroundImage = style;
			}, 5);
		...
		function spotmouseup() {
			...
			style = '-moz-radial-gradient(' + x + 'px ' + y + 'px, circle cover, rgba(0,0,0,0) ' + borderspotlightsize + 'px, ' + lightcolor + ' ' + fullspotlightsize + 'px)';
			spot.style.backgroundImage = style;
...
//----		
	...
	if((calcpartx < 0) &&! (calcparty < 0)){ // X as automatic change view
		...
		window.addEventListener("mousemove",function(event){try {getMouse(window,event);}catch(err){}},false);
...
if(document.height !== undefined) {
    ...
    blurheight = Math.max( B.scrollHeight, B.offsetHeight,H.clientHeight, H.scrollHeight, H.offsetHeight );
				...
				for (var bcount=0; bcount<4; bcount++) {
				newiframeblur.style.mozFilter = 'blur(' + bcount + 'px)';
				}
}
...
/////////// Turn Off the Lights reader slider
	...
	totlreaderbarimg1.setAttribute('src','chrome://TurnOfftheLights/content/images/minimize.png');
	totlreaderbarimg1.addEventListener('click', function (e) {toggle_small()}, true);
	totlreaderbarimg1.addEventListener("mouseover", function (e) {totlreaderbarimg1.setAttribute('src','chrome://TurnOfftheLights/content/images/minimize_on.png');}, false);
	totlreaderbarimg1.addEventListener("mouseout", function (e) {totlreaderbarimg1.setAttribute('src','chrome://TurnOfftheLights/content/images/minimize.png');}, false);
	...
	if(window.devicePixelRatio >= 2){totlreaderbarimagelogo.src = "chrome://TurnOfftheLights/content/icons/icon16@2x.png";}
	else {totlreaderbarimagelogo.src =  "chrome://TurnOfftheLights/content/icons/icon16@2x.png";}
...
(function() {
	...
	update();
			...
			if(hoveroptiondyn5 == true){
			    ...
			    imageObj.src = "chrome://turnoffthelights/content/images/Smoke10.png";
