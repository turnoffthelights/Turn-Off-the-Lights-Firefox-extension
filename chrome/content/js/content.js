...
function addNewStyleSheetByFullCSS() {
var fileref = document.createElement("link");
fileref.setAttribute("rel", "stylesheet");
fileref.setAttribute("type", "text/css");
fileref.setAttribute("href", "chrome://TurnOfftheLights/content/css/light.css");
var h=document.getElementsByTagName("head");if(!h.length)return;
h[0].appendChild(fileref);
}
...
if (lightareoff != null) {
	...
	if ($('stefanvdtheater')){}
		...
		newimg.src = 'chrome://TurnOfftheLights/content/images/theater.jpg';
		newimg.addEventListener('click',function (e) {document.body.removeChild(newimg);},true);
...
shortcutlight = response.shortcutlight;
		...
		if (request.name == "shortcutdefaultopacity") {
		// Run code for Alt+F8
			// Shortcutlight
		...
		if (request.name == "shortcutsaveopacity") {
		// Run code for Alt+F9
			// Shortcutlight
			if(shortcutlight == true){
			if($('stefanvdlightareoff1')){
				var F9saving = Math.round(($('stefanvdlightareoff1').style.opacity)*100);
				firefox.extension.sendRequest({name: 'readersaveme', value : F9saving});
			}
			}
		}
		
		if (request.name == "shortcutopacityincrease") {
		...
		if (request.name == "shortcutopacitydecrease") {
		...
		if (request.name == "shortcutalllightstab") {
		// Run code for Alt+*
			// Shortcutlight
			if(shortcutlight == true){
			// all tabs lights off
			firefox.extension.sendRequest({name: 'emergencyalf'});
			}
		}
		
		if (request.name == "shortcuteyeprotection") {
...
ambilightcolorhex = response.ambilightcolorhex;
ambilight4color = response.ambilight4color;
ambilight1colorhex = response.ambilight1colorhex;
ambilight2colorhex = response.ambilight2colorhex;
ambilight3colorhex = response.ambilight3colorhex;
ambilight4colorhex = response.ambilight4colorhex;
...
var gracePeriod = 250, lastEvent = null, timeout = null;
	...
	function shadesOff(player) {
		...
		var blackon = $('stefanvdlightareoff1');
			...
			godelay = window.setTimeout(function(){
				if (blackon) {firefox.extension.sendRequest({name: "automatic"});}
				else {} // do nothing
	...
	function shadesOn(player) {
		...
		var blackon = $('stefanvdlightareoff1');
			...
			else {firefox.extension.sendRequest({name: "automatic"});}		
			if(autoplaydelay == true){
...
newvcparta3btn.addEventListener("click", function(){
	...
	if(onevideo){
		if(currentvideostepfilter == 0){
			...
			onevideo.style.filter = "grayscale(1)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolgrayscale;
		}
			...
			getstefanvdvideotoolrange.max = "1";
			getstefanvdvideotoolrange.value = "1";
			onevideo.style.filter = "sepia(1)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolsepia;
			}
			...
			getstefanvdvideotoolrange.max = "1";
			getstefanvdvideotoolrange.value = "1";
			onevideo.style.filter = "invert(1)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolinvert;
			}
			...
			onevideo.style.filter = "contrast(10)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolcontrast;
			}
			...
			getstefanvdvideotoolrange.max = "10";
			getstefanvdvideotoolrange.value = "10";
			onevideo.style.filter = "saturate(10)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolsaturate;
			}
			...
			onevideo.style.filter = "hue-rotate(90deg)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolhueroration;
			}
			...
			onevideo.style.filter = "brightness(1.5)"; currentvideostepfilter += 1; newvcpartiaspan.textContent = i18ntitelvideotoolbrightness;
			}
			...
			getstefanvdvideotoolrange.max = "10";
			getstefanvdvideotoolrange.value = "1";
			onevideo.style.filter = ""; currentvideostepfilter = 0; newvcpartiaspan.textContent = i18ntitelvideotoolnormal;
			}
		}
	}, false);
...
if (filtertype == "grayscale") { onevideo.style.filter = "" + filtertype + "(" + gsvtrange + ")"; }
else if(filtertype == "sepia"){onevideo.style.filter = "" + filtertype + "("+gsvtrange+")";}
else if(filtertype == "invert"){onevideo.style.filter = "" + filtertype + "("+gsvtrange+")";}
else if(filtertype == "contrast"){onevideo.style.filter = "" + filtertype + "("+gsvtrange+")";}
else if(filtertype == "saturate"){onevideo.style.filter = "" + filtertype + "("+gsvtrange+")";}
else if(filtertype == "hue-rotate"){onevideo.style.filter = "" + filtertype + "("+gsvtrange+"deg)";}
else if(filtertype == "brightness"){onevideo.style.filter = "" + filtertype + "("+gsvtrange+")";}
}, false);
...
if((ecosaver == true) && (eyen == true)){firefox.extension.sendRequest({name: "automatic"});}
...
if(typeof excludedDomains == "string") {
	...
	for(var domain in excludedDomains)
		...
		for(var i = 0; i < eyebuf.length; i++){
			if(eyechecklistwhite == true){
				if(currenturl == eyebuf[i]){firefox.extension.sendRequest({name: "automatic"});}
			}
	...
	if(eyechecklistblack == true){
		if(eyerabbit == false){firefox.extension.sendRequest({name: "automatic"});eyerabbit = false;}
	}
}
}
...
function ambilightfunction(){
		...
		if (window.location.href.match(/((http:\/\/(.*youtube\.com\/.*))|(https:\/\/(.*youtube\.com\/.*)))/i)){
		var yttest = $("movie_player")/* || $("movie_player-flash")*/; item = 1;
		...
		if(yttest){
		yttest = yttest.wrappedJSObject; // unsafeWindow context
		if (yttest.getPlayerState() == 1) { drawImage(youtubemovieplayer, item); }
		else { drawImage(youtubemovieplayer, item); }
		}
		}
...
try {
	var ayttest = $("movie_player");if(ayttest)ayttest = ayttest.wrappedJSObject; // unsafeWindow context
	if(playerid.paused || playerid.ended || ayttest.getPlayerState() == 0 || ayttest.getPlayerState() == 2){
	// animation go out
...
var downhex4 = hex4; if(!hex4){ hex4 = "#000000"; } // previous value
	...
	if (window.location.href.match(/((http:\/\/(.*youtube\.com\/.*))|(https:\/\/(.*youtube\.com\/.*)))/i)){
		...
		youtubewindow.style.overflow = "visible"; youtubewindow.style.zIndex = 1000;
	}else{
		if(ambilightvarcolor == true){
...
firefox.extension.sendRequest({name: 'nmcustomx', value : getnmcx});
firefox.extension.sendRequest({name: 'nmcustomy', value : getnmcy});
}
}
},
drag:function(e){
